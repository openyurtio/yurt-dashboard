...
repoName=yurt-dashboard
chartName=yurt-dashboard
...
if [ -z "$(git status --porcelain)" ]; then
  ...
  git add .
  echo "align with charts ${repoName}/charts/${chartName} $VERSION from commit $COMMIT_ID"
  git commit -m "align with charts ${repoName} from commit $COMMIT_ID"
  #git tag "$VERSION"
