...
describe("Login Form Test", () => {
  ...
  it("should receive login initial props", () => {
    ...
    const { getByLabelText } = render(
      <BrowserRouter>
        <LoginForm initState={initState} />
      </BrowserRouter>
    );

    expect(getByLabelText("phonenumber-input").value).toBe(
      initState.spec.mobilephone
    );
    expect(getByLabelText("password-input").value).toBe(initState.spec.token);
  });

  it("should accept nonstandard initial props", () => {
    // a nonstandard initial props may be passed when register fail
    const initState = {};
    render(
      <BrowserRouter>
        <LoginForm initState={initState} />
